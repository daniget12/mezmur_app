...
void main() {
  final rawText = r'''Maqaa Abbaa Kan Ilmaa afuura Qulqulluu Waaqa Tokkotti Ni
                                              Amanna Ameen!!!
...
Sittaan dhisee an waa’ee koo''';
  ...
  final amharicWords = [
    'tenagera', 'izara', 'egziabiher', 'igziabiher', 'geta', 'amlak', 'mariam',
    'dingil', 'kibra', 'kidusan', 'selam', 'yimesgen', 'medanialem', 'hawwii',
    'sillaasee', 'mikael', 'gabriel', 'aleluya', 'hiwot', 'tesfa', 'tsidk', 'kidan', 'selamawit', 'haymanot',
    'mariam', 'selam', 'alemin', 'awajun', 'sibkat', 'mistir', 'getachin', 'amlakin', 'medan', 'tabotu'
  ];

  final amharicIndicators = [
    ' yale ', ' yes ', ' nw ', ' liyu ', ' geta ', ' egziabiher ', ' igziabiher ', ' dingil ', ' kidusan ', ' inat ', ' amlak ', ' ba ', ' ye ', ' le '
  ];
  
  final oromoIndicators = [
    ' waaqayyoo ', ' gooftaa ', ' faaruu ', ' isaa ', ' koo ', ' kee ', ' nuuf ', ' sanaa ', ' nagaa ', ' garuu ', ' kana ', ' dhugaa ', ' keessaa '
  ];
  ...
  String guessLanguage(String text) {
    ...
    for (var ind in amharicIndicators) {
      if (lowerText.contains(ind)) aCount++;
    }
    for (var ind in oromoIndicators) {
      if (lowerText.contains(ind)) oCount++;
    }
    
    int amharicScore = 0;
    for (var word in amharicWords) {
      if (lowerText.contains(word)) amharicScore++;
    }

    if (aCount > oCount + 1) return 'Amharic';
    if (oCount > aCount + 1) return 'Afaan Oromo';
    return 'Afaan Oromo';
  }

  // Find where the actual content starts, which is after the table of contents.
  // The table of contents ends at 174, so we can find the second occurrence of "1." or similar.
  final startOfContentIndex = rawText.indexOf('1.ARGADHEEN JIRA!!!');

  // Clean up some known typos
  final actualContent = (startOfContentIndex != -1 ? rawText.substring(startOfContentIndex) : rawText)
      .replaceAll('1O3.', '103.')
      .replaceAll('110,', '110.')
      .replaceAll('169.yegna newu', '168. Yegna newu')
      .replaceAll('159 Kirstos', '159. Kirstos')
      .replaceAll('162 Attii', '162. Attii');

  // Regex to find mezmur sections (allowing leading spaces and dots)
  final pattern = RegExp(r'^\s*(\d+)\s*\.\s*(.+?)(?:!!!)?\s*$', multiLine: true);
  final matches = pattern.allMatches(actualContent).toList();

  final mezmurs = <Map<String, dynamic>>[];

  for (int i = 0; i < matches.length; i++) {
    final match = matches[i];
    final num = int.parse(match.group(1)!);
    var title = match.group(2)!.trim();
    // Strip trailing exclamation marks
    title = title.replaceAll(RegExp(r'!+$'), '').trim();

    final startIdx = match.end;
    final endIdx = (i + 1 < matches.length) ? matches[i + 1].start : actualContent.length;
    final content = actualContent.substring(startIdx, endIdx).trim();
    ...
    final lang = guessLanguage('$title $content');
  ...
  print('Found ${mezmurs.length} mezmurs.');

  mezmurs.sort((a, b) {
    final langA = a['language'] == 'Afaan Oromo' ? 0 : 1;
    final langB = b['language'] == 'Afaan Oromo' ? 0 : 1;
  ...
  buffer.writeln("import '../models/mezmur.dart';");
  buffer.writeln();
  buffer.writeln("final List<Mezmur> mezmurs = [");

  for (int idx = 0; idx < mezmurs.length; idx++) {
    final mezmur = mezmurs[idx];
    
    // Properly escape content for Dart raw string
    var contentEscaped = mezmur['content'].toString();
    var titleEscaped = mezmur['title'].toString().replaceAll("'", "\\'");
    
    buffer.writeln("  Mezmur(");
    buffer.writeln("    id: '${idx + 1}',");
    buffer.writeln("    title: '$titleEscaped',");
    buffer.writeln("    language: '${mezmur['language']}',");
    buffer.writeln("    fullText: r'''$contentEscaped''',");
    buffer.writeln("  ),");
  }

  buffer.writeln("];");

  final file = File(r'c:\Users\danie\.gemini\antigravity\scratch\mezmur_app\lib\data\mezmur_data.dart');
  file.writeAsStringSync(buffer.toString());
  
  print('Successfully wrote ${file.path}');
}
